...
def test_search_book_by_valid_isbn(mock_db_session):
    book = Book(title="Test Book", author="Author", pages=123, isbn="1234567890")
    mock_db_session.query.return_value.filter.return_value.first.return_value = book
    response = client.get("/books/search/1234567890")
    assert response.status_code == 200
    assert response.json()["title"] == "Test Book"

def test_search_book_by_invalid_isbn(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.first.return_value = None
    response = client.get("/books/search/invalidisbn")
    assert response.status_code == 404
...
def test_add_book_to_basket(mock_db_session):
    book = Book(title="Test Book", author="Author", pages=123, isbn="1234567890")
    mock_db_session.query.return_value.filter.return_value.first.return_value = book
    response = client.post(f"/basket/add/{book.id}")
    assert response.status_code == 200
    assert response.json()["detail"] == "Book added to your basket"
