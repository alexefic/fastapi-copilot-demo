...
def test_search_books(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.all.return_value = [
        Book(id=1, title="Example Book", author="John Doe", pages=300, genre="Fiction"),
        Book(id=2, title="Another Example Book", author="Jane Doe", pages=250, genre="Fiction")
    ]
    response = client.get("/books/search?genre=Fiction")
    assert response.status_code == 200
    assert len(response.json()) == 2
    assert response.json()[0].get("title") == "Example Book"
    assert response.json()[1].get("title") == "Another Example Book"
