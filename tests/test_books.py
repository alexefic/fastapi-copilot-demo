...
def test_get_top_selling_fantasy_books(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.order_by.return_value.limit.return_value.all.return_value = [
        Book(id=1, title="Fantasy Book 1", author="Author 1", pages=300, genre="Fantasy", sales=1000),
        Book(id=2, title="Fantasy Book 2", author="Author 2", pages=250, genre="Fantasy", sales=900)
    ]
    response = client.get("/books/top-selling/fantasy")
    assert response.status_code == 200
    assert len(response.json()) == 2
    assert response.json()[0].get("title") == "Fantasy Book 1"
    assert response.json()[1].get("title") == "Fantasy Book 2"
