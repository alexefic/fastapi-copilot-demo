...
def test_create_book(mock_db_session):
    ...
    assert response.json().get("isbn") == "9781234567897"

def test_read_book_success(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.first.return_value = Book(id=1, title="Test Book", author="Test Author", pages=100, isbn="9781234567897")
...
def test_update_book_success(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.first.return_value = Book(id=1, title="Old Title", author="Old Author", pages=100, isbn="9781234567897")
...
def test_search_items_by_isbn_success(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.all.return_value = [
        Book(id=1, title="Test Book", author="Test Author", pages=100, isbn="9781234567897")
    ]
    ...
    assert response.json()[0].get("isbn") == "9781234567897"

def test_search_items_by_isbn_not_found(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.all.return_value = []
    response = client.get("/search?isbn=9781234567897")
    assert response.status_code == 404
    assert response.json().get("detail") == "No items found"

def test_search_items_by_isbn_invalid_format(mock_db_session):
    response = client.get("/search?isbn=invalid_isbn")
    assert response.status_code == 400
    assert response.json().get("detail") == "Invalid ISBN format"

def test_search_items_by_isbn_empty(mock_db_session):
