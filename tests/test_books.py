...
def test_search_books():
    response = client.get("/books/search?author=J.K.%20Rowling")
    assert response.status_code == 200
    assert len(response.json()["books"]) > 0

def test_add_book_to_wishlist():
    search_response = client.get("/books/search?author=J.K.%20Rowling")
    book_id = search_response.json()["books"][0]["id"]
    add_response = client.post(f"/wishlist/{book_id}")
