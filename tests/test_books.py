...
def test_search_books(mock_db_session):
    ...
    response = client.get("/books/search?author_name=Author Name")
    assert response.status_code == 200
    assert len(response.json()) == 2
    assert response.json()[0].get("title") == "Book Title 1"
    assert response.json()[1].get("title") == "Book Title 2"

def test_add_to_wishlist(mock_db_session, mock_current_user):
    mock_current_user.id = 1
    response = client.post("/wishlist", json={"book_id": 1}, headers={"Authorization": f"Bearer {mock_current_user.token}"})
