...
def test_add_author_to_book_success(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.first.return_value = Book(id=1, title="Test Book", author="Old Author", pages=100)
    response = client.post("/books/1/author", json={"author": "New Author"})
    assert response.status_code == 200
    assert response.json().get("author") == "New Author"

def test_add_author_to_book_not_found(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.first.return_value = None
    response = client.post("/books/1/author", json={"author": "New Author"})
    assert response.status_code == 404
    assert response.json().get("detail") == "Book not found"
