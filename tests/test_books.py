...
def test_add_book_to_basket_success(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.first.return_value = Book(id=1, title="Test Book", author="Test Author", pages=100)
    response = client.post("/basket/add", json={"book_id": 1})
    assert response.status_code == 200
    assert response.json().get("detail") == "Book added to basket"

def test_add_book_to_basket_not_found(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.first.return_value = None
    response = client.post("/basket/add", json={"book_id": 1})
    assert response.status_code == 404
    assert response.json().get("detail") == "Book not found"
