...
def test_search_books_success(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.all.return_value = [
        Book(id=1, title="Children's Book Title 1", author="Author 1", pages=100, category="children"),
        Book(id=2, title="Children's Book Title 2", author="Author 2", pages=150, category="children")
    ]
    response = client.get("/books/search?category=children")
    assert response.status_code == 200
    assert len(response.json()) == 2
    assert response.json()[0].get("title") == "Children's Book Title 1"
    assert response.json()[1].get("title") == "Children's Book Title 2"

def test_search_books_not_found(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.all.return_value = []
    response = client.get("/books/search?category=children")
    assert response.status_code == 404
    assert response.json().get("detail") == "No books found in the 'Children' category"
