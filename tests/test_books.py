...
def test_add_to_wishlist_success(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.first.return_value = None
    ...
    assert response.status_code == 200
    assert response.json().get("detail") == "Book has been added to your wishlist"

def test_add_to_wishlist_already_exists(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.first.return_value = Wishlist(id=1, user_id="user123", book_id=1)
    response = client.post("/wishlist", json={
        "user_id": "user123",
        "book_id": 1
    })
    assert response.status_code == 400
    assert response.json().get("detail") == "Book already in wishlist"
