...
def test_read_book_success(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.first.side_effect = [
        Book(id=1, title="Test Book", author_id=1, pages=100),
        Author(id=1, name="Test Author", bio="Test Bio")
    ]
...
def test_update_book_success(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.first.side_effect = [
        Book(id=1, title="Old Title", author_id=1, pages=100),
        Author(id=1, name="Old Author", bio="Old Bio")
    ]
    ...
    assert response.json().get("author")["name"] == "New Author"
    assert response.json().get("author")["bio"] == "New Bio"
...
def test_delete_book_success(mock_db_session):
    ...
    assert response.status_code == 200
    assert response.json().get("message") == "Book and associated author deleted successfully"
