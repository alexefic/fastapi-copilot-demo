...
def test_search_books_success(mock_db_session):
    ...
    assert response.json().get("items")[0].get("title") == "Test Book"
    assert response.json().get("items")[0].get("author") == "Test Author"
    assert response.json().get("items")[0].get("pages") == 100
    assert response.json().get("items")[0].get("isbn") == "1234567890"
