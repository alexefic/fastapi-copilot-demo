...
def test_read_book_by_isbn_not_found(mock_db_session):
    ...
    response = client.get("/books/isbn/1234567890")
    assert response.status_code == 404
    assert response.json().get("detail") == "No book found for the entered ISBN"


def test_read_book_by_isbn_empty_isbn(mock_db_session):
    response = client.get("/books/isbn/")
    assert response.status_code == 400
    assert response.json().get("detail") == "Please enter an ISBN number"
