...
def test_search_books_by_title(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.all.return_value = [
        Book(id=1, title="Test Book", author="Test Author", pages=100)
    ]
    response = client.get("/books/search/title?title=Test")
    assert response.status_code == 200
...
def test_search_books_by_author(mock_db_session):
    ...
    response = client.get("/books/search/author?author=Test")
    assert response.status_code == 200
    assert len(response.json()) == 1
    assert response.json()[0].get("author") == "Test Author"

def test_search_books_by_isbn(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.all.return_value = [
        Book(id=1, title="Test Book", author="Test Author", pages=100)
    ]
    response = client.get("/books/search/isbn?isbn=1234567890")
    assert response.status_code == 200
    assert len(response.json()) == 1
