...
def test_search_books(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.all.return_value = [
        Book(id=1, title="Example Book", author="John Doe", pages=300),
        Book(id=2, title="Another Example Book", author="Jane Doe", pages=250)
    ]
    response = client.get("/books/search?title=example")
    assert response.status_code == 200
    assert len(response.json().get("books")) == 2
    assert response.json().get("books")[0].get("title") == "Example Book"
    assert response.json().get("books")[1].get("title") == "Another Example Book"
