...
def test_search_books_by_author(mock_db_session):
    mock_db_session.query.return_value.filter.return_value.all.return_value = [
        Book(id=1, title="Test Book 1", author="Test Author", pages=100),
        Book(id=2, title="Test Book 2", author="Test Author", pages=150)
    ]
    response = client.get("/books/search?author=Test Author")
    assert response.status_code == 200
    books = response.json().get("books")
    assert len(books) == 2
    assert books[0].get("title") == "Test Book 1"
    assert books[1].get("title") == "Test Book 2"

def test_add_book_to_wishlist(mock_db_session):
    response = client.post("/wishlist", json={"book_id": 1})
    assert response.status_code == 200
    assert response.json().get("message") == "Book added to your wishlist"
