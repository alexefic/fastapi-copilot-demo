...
def test_read_book_success():
    with patch('fastapi_demo.routers.books.get_db', return_value=MagicMock()) as mock_db:
        mock_db.return_value.query.return_value.filter.return_value.first.return_value = Book(id=1, title="Test Book", author="Test Author", pages=100)
...
def test_update_book_success():
    with patch('fastapi_demo.routers.books.get_db', return_value=MagicMock()) as mock_db:
        mock_db.return_value.query.return_value.filter.return_value.first.return_value = Book(id=1, title="Old Title", author="Old Author", pages=100)
