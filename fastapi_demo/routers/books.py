...
@router.get("/books/search", response_model=BookInfo, dependencies=[Depends(RateLimiter(times=5, seconds=60))])
def search_book_by_isbn(
    isbn: str = Query(..., description="The ISBN of the book to be searched", example="9781234567897"),
    db: Session = Depends(get_db)):
    if not isbn:
        raise HTTPException(status_code=400, detail="Enter valid ISBN")
    book = db.query(Book).filter(Book.isbn == isbn).first()
    if book is None:
        raise HTTPException(status_code=404, detail="No items found for the entered ISBN")
