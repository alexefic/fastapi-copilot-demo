...
@router.get("/books/search", summary="Search book by ISBN", description="This endpoint searches for a book by its ISBN and returns the book details and read status.")
def search_book_by_isbn(isbn: str = Query(..., description="The ISBN number of the book to search for"), db: Session = Depends(get_db)):
    book = db.query(Book).filter(Book.isbn == isbn).first()
    if book is None:
        raise HTTPException(status_code=404, detail="Book not found")
    # Assuming a function `has_user_read_book` exists to check read status
    read_status = has_user_read_book(book.id)
    return {"id": book.id, "title": book.title, "author": book.author, "pages": book.pages, "read": read_status}
