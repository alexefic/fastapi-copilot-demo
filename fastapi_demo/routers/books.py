...
def search_books(
    title: Optional[str] = Query(None, description="The title of the book to search for"),
    author: Optional[str] = Query(None, description="The author of the book to search for"),
    isbn: Optional[str] = Query(None, description="The ISBN of the book to search for"),
    db: Session = Depends(get_db)):
    query = db.query(Book)
    if title:
