...
@router.get("/books/top-selling/fantasy", response_model=List[BookInfo], dependencies=[Depends(RateLimiter(times=100, seconds=60))])
def get_top_selling_fantasy_books(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    # Verify token and user permissions here
    books = db.query(Book).filter(Book.genre == 'Fantasy').order_by(Book.sales.desc()).limit(10).all()
