...
from ..utils import validate_isbn, log_deletion_activity
...
@router.delete("/books/author/{isbn}", status_code=status.HTTP_200_OK)
def delete_books_by_author(isbn: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    # Validate ISBN
    ...
    books_to_delete = db.query(Book).filter(Book.author == author).all()
    ...
    log_deletion_activity(current_user.id, isbn, books_to_delete)
    return {"detail": "All books by the author have been deleted."}