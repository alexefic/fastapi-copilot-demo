...
@router.get("/books/search", response_model=List[BookInfo],
    ...
    description="This endpoint searches for books by genre and returns a list of matching books",
    response_description="A list of books matching the search criteria")
def search_books_by_genre(
    genre: str = Query(..., description="The genre of the book to search for"),
    db: Session = Depends(get_db)):
