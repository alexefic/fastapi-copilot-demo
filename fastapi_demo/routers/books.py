...
ISBN_REGEX = re.compile(r'^[0-9]{10}([0-9]{3})?$')
...
def search_books_by_isbn(isbn: str = Query(..., description="The ISBN of the book to be searched"), db: Session = Depends(get_db)):
    if not isbn:
        raise HTTPException(status_code=400, detail="Please enter an ISBN")
    if not ISBN_REGEX.match(isbn):
