...
@router.get("/books/search/title", response_model=List[BookInfo])
def search_books_by_title(title: str = Query(..., description="The title of the book to search for"), db: Session = Depends(get_db)):
    books = db.query(Book).filter(Book.title.ilike(f"%{title}%")).all()
    return books

@router.get("/books/search/author", response_model=List[BookInfo])
def search_books_by_author(author: str = Query(..., description="The author of the book to search for"), db: Session = Depends(get_db)):
    books = db.query(Book).filter(Book.author.ilike(f"%{author}%")).all()
    return books

@router.get("/books/search/isbn", response_model=List[BookInfo])
def search_books_by_isbn(isbn: str = Query(..., description="The ISBN of the book to search for"), db: Session = Depends(get_db)):
