...
def search_book(isbn: str = Query(..., description="The ISBN of the book to be searched"), db: Session = Depends(get_db)):
...
def add_to_wishlist(wishlist: WishlistCreate, db: Session = Depends(get_db)):
    wishlist_entry = db.query(Wishlist).filter(Wishlist.user_id == wishlist.user_id, Wishlist.book_id == wishlist.book_id).first()
    if wishlist_entry:
        raise HTTPException(status_code=400, detail="Book already in wishlist")
    new_entry = Wishlist(user_id=wishlist.user_id, book_id=wishlist.book_id)
    db.add(new_entry)
    db.commit()
    return {"detail": "Book has been added to your wishlist"}
