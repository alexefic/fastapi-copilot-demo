...
@router.get("/books/search/{isbn}", response_model=BookInfo, dependencies=[Depends(RateLimiter(times=5, seconds=60))])
...
@router.post("/basket/add/{book_id}", response_model=dict)
def add_book_to_basket(book_id: int, db: Session = Depends(get_db)):
    book = db.query(Book).filter(Book.id == book_id).first()
    if book is None:
        raise HTTPException(status_code=404, detail="Book not found")
    # Logic to add the book to the user's basket
    return {"detail": "Book added to your basket"}
