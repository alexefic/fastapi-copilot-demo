...
@router.get("/books/search", response_model=List[BookInfo], dependencies=[Depends(RateLimiter(times=5, seconds=60))])
def search_books_by_isbn(
    isbn: str = Query(..., description="The ISBN of the book to search for", min_length=10, max_length=13),
    db: Session = Depends(get_db)):
    logger.info(f"Search query for ISBN: {isbn}")
    if not isbn:
